...
class Restart(Module):
    ...
    def _build_uptodate_message(
        ...
        body = (
            ...
            f"<code>Status   : Already up to date ✅</code>\n"
            f"<code>Branch   : {html.escape(branch)}</code>\n"
            f"<code>Commit   : {html.escape(commit_hash)}</code>\n"
            f"<code>Worktree : {html.escape(worktree_state)}</code>\n\n"
            f"<b>Pull Output</b>\n"
    ...
    def _build_updated_message(
        ...
        body = (
            f"<b>Update Summary</b>\n\n"
            f"<code>Status   : Updated ✅</code>\n"
            f"<code>Branch   : {html.escape(branch)}</code>\n"
            f"<code>From     : {html.escape(old_hash)}</code>\n"
            f"<code>To       : {html.escape(new_hash)}</code>\n"
            f"<code>Commits  : {len(commits)}</code>\n"
            f"<code>Changes  : {html.escape(diff_summary)}</code>\n"
            f"<code>Worktree : {html.escape(worktree_state)}</code>\n\n"
            f"<b>Pull Output</b>\n"
            f"<blockquote expandable>{html.escape(pull_preview)}</blockquote>\n\n"
            f"<b>New Commits</b>\n"
