...
class Restart(Module):
    ...
    def _build_uptodate_message(
        ...
        body = (
            ...
            f"Commit: <code>{html.escape(commit_hash)}</code>\n"
            f"Worktree: <code>{html.escape(worktree_state)}</code>\n\n"
            f"<b>Pull Output</b>\n"
    ...
    def _build_updated_message(
        ...
        body = (
            ...
            f"Status: Updated ✅\n"
            f"Branch: <code>{html.escape(branch)}</code>\n"
            f"From: <code>{html.escape(old_hash)}</code>\n"
            f"To: <code>{html.escape(new_hash)}</code>\n"
            f"Commits: <code>{len(commits)}</code>\n"
            f"Changes: <code>{html.escape(diff_summary)}</code>\n"
            f"Worktree: <code>{html.escape(worktree_state)}</code>\n\n"
            f"<b>Pull Output</b>\n"
            f"<blockquote expandable>{html.escape(pull_preview)}</blockquote>\n\n"
            f"<b>New Commits</b>\n"
