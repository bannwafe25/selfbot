...
class InstaDL(Module):
        ...
        if await asyncio.to_thread(self.session_file.exists):
            try:
                await asyncio.to_thread(
                    self.loader.load_session_from_file, ig_user, str(self.session_file)
                )
    ...
    async def on_message_out(self, event: Message):
        ...
        try:
            if shortcode:
                media_files, caption, temp_dir_path = await self._download_post(
                    shortcode
                )
                media_types = [
            ...
            try:
                ...
                if resp.status_code != 200:
                    raise Exception(f"Fallback API failed with HTTP {resp.status_code}")
                ...
                for item in data["data"]:
                    ...
                    file_resp = await self.client.http.get(file_url, timeout=60)
                    if file_resp.status_code != 200:
                        continue
                    await asyncio.to_thread(tmp_path.write_bytes, file_resp.content)
                ...
                await event.edit(
                    f"<b>Error:</b> Both download methods failed.\n"
                    f"<b>Instaloader:</b> <code>{html.escape(str(e))}</code>\n"
                    f"<b>Fallback:</b> <code>{html.escape(str(fallback_e))}</code>"
                )
                return
        ...
        try:
            ...
            if len(media_files) == 1:
                ...
                await self._send_album_chunks(
                    event.chat.id, media_files, media_types, final_caption, reply_to
                )
                await event.delete()
        finally:
            if temp_dir_path:
